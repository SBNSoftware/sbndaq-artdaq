...
class Product:
    ...
    def manifest_entry(self):
        ...
        pyqual = list(
            filter(
                re.compile(r"(py(2|3)|p2715a|p383b|p392|p3913|p3915)").search,
                qualifiers,
            )
        )
        ...
        tarball = "{0:s}-{1:s}-{2:s}".format(
            name, version.replace("_", ".").replace("v", ""), arch
        )

        if rqual:
            tarball += "-{0:s}".format(rqual[0])
        if pyqual:
            tarball += "-{0:s}".format(pyqual[0])
        if squal:
            tarball += "-{0:s}".format(squal[0])
        if bqual:
            tarball += "-{0:s}".format(bqual[0])

        tarball += ".tar.bz2"
        entry = "{0:20s} {1:10s} {2:70s}".format(name, version, tarball)
        entry += " -f {0:20s}".format(
            "Linux64bit+3.10-2.17" if arch == "slf7-x86_64" else "NULL"
        )

        if (
            self.qualifiers != "-nq-"
            and self.qualifiers != "-"
            and self.qualifiers != "noarch"
        ):
...
class ManifestBuilder:
    ...
    def build_manifest(self):
        ...
        for index, product in self.df_products.iterrows():
            qualifiers = (
                "noarch"
                if product[0] not in product_qualifiers
                else product_qualifiers[product[0]]
            )

            if product[2] not in [np.nan, "-"] and not all(
                q in product_qualifiers["qualifier"].split(":")
                for q in product[2].split(":")
            ):
                continue
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "hp:m:q:", ["product_deps=", "manifest=", "qualifiers="]
        )
