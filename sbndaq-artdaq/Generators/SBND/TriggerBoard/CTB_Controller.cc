...
bool CTB_Controller::send_reset() {
  ...
  //sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Sending a reset" << std::endl;
...
bool CTB_Controller::send_start() {
  ...
  if (! is_conf_) {
    ///sbndaq::DAQLogger::LogError("CTB_Controller") << "ERROR: Can't start a run without configuring first" << std::endl;
    return false ;
  }

  ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Sending a start run" << std::endl;
...
bool CTB_Controller::send_stop() {
  ...
  if ( ! is_running_.load() ) {
    ///sbndaq::DAQLogger::LogWarning("CTB_Controller") << "Stop requested while not runninng" ;
    return true ;
  }

  ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Sending a stop run" << std::endl;
...
bool CTB_Controller::send_config( const std::string & config ) {
  ...
  if ( is_conf_ ) {
    ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Resetting before configuring" << std::endl;
    send_reset();
  }

  ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Sending config" << std::endl;
...
bool CTB_Controller::send_message( const std::string & msg ) {
  ...
  ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Sending message: " << msg << std::endl;
  ...
  ///sbndaq::DAQLogger::LogDebug("CTB_Controller") << "Unformatted answer: " << std::endl << raw_answer.str() << std::endl; 
  ...
  ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Received messages: " << messages.size() << std::endl;
  ...
  for ( Json::Value::ArrayIndex i = 0; i != messages.size(); ++i ) {
    ...
    if ( type.find("error") != std::string::npos || type.find("Error") != std::string::npos || type.find("ERROR") != std::string::npos ) {
     /// sbndaq::DAQLogger::LogError("CTB_Controller") << "Error from the Board: " << messages[i]["message"].asString() << std::endl ;
      ret = false ;
    }
    else if ( type.find("warning") != std::string::npos || type.find("Warning") != std::string::npos || type.find("WARNING") != std::string::npos ) {
      ///sbndaq::DAQLogger::LogWarning("CTB_Controller") << "Warning from the Board: " << messages[i]["message"].asString() << std::endl;
    }
    else if ( type.find("info") != std::string::npos || type.find("Info") != std::string::npos || type.find("INFO") != std::string::npos) {
      ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Info from the board: " << messages[i]["message"].asString() << std::endl;
    }
    else {
      std::stringstream blob;
      blob << messages[i] ;
      ///sbndaq::DAQLogger::LogInfo("CTB_Controller") << "Unformatted from the board: " << blob.str() << std::endl;
    }
  }
