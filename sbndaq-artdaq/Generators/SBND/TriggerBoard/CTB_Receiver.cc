...
int CTB_Receiver::_word_receiver() {
  ...
  ///sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "Header size: " << header_size << std::endl
  ...
  ///sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "Watiting for an incoming connection on port " << _port << std::endl;
  ...
  ///sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "Connection received: start reading" << std::endl;
  ...
  while ( ! _stop_requested.load() ) {
    ...
    //    sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "Package size "  << n_bytes << std::endl ;
	...
	_calibration_file.flush() ;
        //sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "Word Type: " << temp_word.frame.word_type << std::endl ;
...
/*        ///sbndaq::DAQLogger::LogError("CTB_Receiver") << "Feedback word: " << std::endl
						  << std::hex 
...
*/
      ...
      while ( ! _word_buffer.push( temp_word ) && ! _stop_requested.load() ) {
        ///sbndaq::DAQLogger::LogWarning("CTB_Receiver") << "Word Buffer full and cannot store more" << std::endl ;
      }
  ...
  if ( _error_state.load() ) {
    ...
    if ( closing_error ) {
      ///sbndaq::DAQLogger::LogError("CTB_Receiver") << "Error in shutdown: " << closing_error.message() << std::endl ;
    }
  }

  socket.close( closing_error ) ;
  if ( closing_error ) {
    ///sbndaq::DAQLogger::LogError("CTB_Receiver") << "Socket closing failed: " << closing_error.message() << std::endl ;
  }

  ///sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "Connection closed: stop receiving data from the CTB" << std::endl ;
...
bool CTB_Receiver::IsTSWord( const ptb::content::word::word_t & w ) noexcept {
  ...
  //sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "word type " <<  w.frame.word_type  << std::endl ;
...
void CTB_Receiver::_init_calibration_file() {
  ...
  ///sbndaq::DAQLogger::LogDebug("CTB_Receiver") << "New Calibration Stream file: " << global_name << std::endl ;
...
bool CTB_Receiver::_read( T & obj , boost::asio::ip::tcp::socket & socket ) {
  ...
  if ( receiving_error == boost::asio::error::eof) {
    ...
    ///sbndaq::DAQLogger::LogInfo("CTB_Receiver") << "Socket closed: "<< receiving_error.message()  << std::endl ;
    return false ;
  }
  
  if ( receiving_error ) {
    ///sbndaq::DAQLogger::LogError("CTB_Receiver") <<"Read failure: " << receiving_error.message() << std::endl ;
    return false ;
  }
