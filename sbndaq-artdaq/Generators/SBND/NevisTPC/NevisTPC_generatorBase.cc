...
bool sbndaq::NevisTPC_generatorBase::FillFragment(artdaq::FragmentPtrs &frags, bool clear_buffer[[gnu::unused]]){
  ...
  */
  ...
  TRACE(TFILLFRAG,"Created fragment with sequenceID=%lu, fragmentID=%u, TimeStamp=%lu, Framenumber=%u",
	frags.back()->sequenceID(),frags.back()->fragmentID(),frags.back()->timestamp(), header->getFrameNum());
  //  TRACE(TFILLFRAG,"TPC frame number: " header->getFrameNum() 
