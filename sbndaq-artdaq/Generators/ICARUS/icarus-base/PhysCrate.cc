...
void PhysCrate::initialize(std::vector<int> busVec, std::vector<unsigned int> boardsinCrateVec, unsigned int countdown)
{
  ...
  for (int iBoard = 0; iBoard < nBoards; ++iBoard)
    BoardIDs[iBoard] = boards[iBoard]->boardId;
...
void PhysCrate::configure(BoardConf conf)
{
  for (int i = 0; i < nBoards; i++)
  {
...
void PhysCrate::configureTrig(TrigConf conf)
{
  for (int i = 0; i < nBoards; i++)
  {
