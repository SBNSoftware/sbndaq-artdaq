	...
	if (t != ot) { // So you don't get debug info until next unix timestamp second
		...
		*/
		...
		if (diag_result & 1) {
			...
			TRACE(TLVL_DEBUG,"resending: %d", ser_no);
		}
		...
		if (diag_result & 2) {
			...
			TRACE(TLVL_DEBUG,"usb_full: %d",ser_no);
		}
		if (diag_result & 4) {
			//printf("pmt_full: %d\n",ser_no);
			//sprintf(gaibu_debug_msg,"pmt_full: %d",ser_no);
			//gaibu_msg(MEXCEPTION, gaibu_debug_msg);
			TRACE(TLVL_DEBUG,"pmt_full: %d",ser_no);
		}
		if (!(diag_result & 8)) {
			//printf("no_lock: %d\n",ser_no);
			//sprintf(gaibu_debug_msg,"no_lock: %d",ser_no);
			//gaibu_msg(MEXCEPTION, gaibu_debug_msg);
			TRACE(TLVL_DEBUG,"no_lock: %d",ser_no);
		}
		if (!(diag_result & 16)) {
			//printf("no_clk: %d\n",ser_no);
			//sprintf(gaibu_debug_msg,"no_clk: %d",ser_no);
			//gaibu_msg(MEXCEPTION, gaibu_debug_msg);
			TRACE(TLVL_DEBUG,"no_clk: %d",ser_no);
		}
		//
...
static void cb_diag_in(struct libusb_transfer *transfer) {
	...
	switch(transfer->status) {
		...
		default:
			...
			TRACE(TLVL_DEBUG,"%d diag in error %d", ser_no, transfer->status); 
			kill_me = 1;
			len = 0;
			break;
	}
...
static void check_diag_in() {
	...
	if (r < 0) {
		...
		TRACE(TLVL_DEBUG,"com in: submit error"); 
		kill_me = 1;
	} else {
		diag_in_ready = 0;
	}
...
static void cb_data_in(struct libusb_transfer *transfer)
{
	...
	switch(transfer->status) {
		...
		default:
			...
			TRACE(TLVL_DEBUG,"%d data in error %d", ser_no, transfer->status); 
			kill_me = 1;
			p->len = 0;
			break;
	}
...
int try_to_open(int file) {
  ...
  if(file<2) {
	...
	TRACE(TLVL_DEBUG,"Error naming file for USB %d, time now=%ld, time_pred=%ld, get auto name",ser_no,time(0),time_lastopen[ser_no]+latency);
//	t = time_lastopen[ser_no] + latency;
  ...
  if (fd < 0) {
    ...
    //gaibu_msg(MNOTICE, gaibu_msg_buf);
    TRACE(TLVL_DEBUG,"Error creating file for USB %d",ser_no);
    return 0;
  }
...
void try_to_close(int file) {
  ...
  if (res) {
    ...
    TRACE(TLVL_DEBUG,"Error closing file for USB %d",ser_no);
    return;
  }
  ...
  if (res) { 
        ...
        //gaibu_msg(MNOTICE, mymsg);
	TRACE(TLVL_DEBUG,"Error cound not rename file for USB %d",ser_no);
...
static void check_data_in() {
  ...
  if (len >= 0) {
    ...
    tmp = *((int *) msg_buf.mdata);
    //printf("Inhibit: %d\n",tmp);
    TRACE(TLVL_DEBUG,"Inhibit: %d",tmp); 
    ...
    if(tmp == 0) { 
      ...
      //gaibu_msg(MNOTICE, mymsg);
      TRACE(TLVL_DEBUG,"USB %d: releasing inhibit",ser_no);
      ...
      //gaibu_msg(MNOTICE, mymsg);
      TRACE(TLVL_DEBUG,"USB %d: inhibiting writing to file",ser_no);
      ...
      //gaibu_msg(MNOTICE, mymsg);
      TRACE(TLVL_DEBUG,"USB %d: releasing inhibit for baseline",ser_no);
...
static void read_data_in() {
  ...
  if (d[rw].buf_state == BUF_READING) {
	...
	//gaibu_msg(MNOTICE, gaibu_msg_buf);
	TRACE(TLVL_DEBUG,"Error deleting data IN for USB %d",ser_no);
      }
  ...
  while (d[rw].buf_state == BUF_DONE) {
	...
	if( inhibit_writetofile == 0) {
	  ...
	  //gaibu_msg(MNOTICE, gaibu_msg_buf);
	  TRACE(TLVL_DEBUG,"Error: no file to write to for USB %d",ser_no);
	...
	*/
	...
	if (ret) {
	  ...
	  //gaibu_msg(MNOTICE, gaibu_msg_buf);
	  TRACE(TLVL_DEBUG,"aio write submit error for USB %d",ser_no);
	  kill_me = 1;
	  d[rw].buf_state = BUF_NOTHING;
	} else {
  ...
  while (1) {
	...
	//gaibu_msg(MNOTICE, gaibu_msg_buf);
	TRACE(TLVL_DEBUG,"aio error1 %d",errno);
	kill_me = 1; //// for now
      }
    ...
    default:
      ...
      //gaibu_msg(MNOTICE, gaibu_msg_buf);
      TRACE(TLVL_DEBUG,"aio error2 %d",errno);
      kill_me = 1; //// for now
      break;
    }
  }
  ...
  while (d[nr].buf_state == BUF_NOTHING) { 
		...
		if (r < 0) {
		  ...
		  //gaibu_msg(MNOTICE, gaibu_msg_buf);
		  TRACE(TLVL_DEBUG,"data in submit error for USB %d",ser_no);
		  kill_me = 1;
		  break;		// keep trying?
		} else {
...
static void cb_com_out(struct libusb_transfer *transfer) {
  ...
  if (transfer->status == LIBUSB_TRANSFER_COMPLETED) {
    ...
    //gaibu_msg(MNOTICE, gaibu_msg_buf);
    TRACE(TLVL_DEBUG,"com out transfer status %d", transfer->status);
    kill_me = 1;
  }
  com_out_ready = 1;
}
 ...
 cco_resubmit:
  if (com_out_attempt) {
    ...
    if (r < 0) {
      ...
      //gaibu_msg(MNOTICE, gaibu_msg_buf);
      TRACE(TLVL_DEBUG,"com out: submit error for USB %d", ser_no);
      kill_me = 1;
    } else {
      com_out_ready = 0;
    }
...
static void check_per_out() {
  ...
  if (r < 0) {
    ...
    //gaibu_msg(MNOTICE, gaibu_msg_buf);
    TRACE(TLVL_DEBUG,"per out: submit error for USB");
    kill_me = 1;
  } else {
    per_out_ready = 0;
  }
...
int main(int argc, char **argv)
{
	...
	if (r < 0) {
		...
		//gaibu_msg(MNOTICE, gaibu_msg_buf);
		TRACE(TLVL_DEBUG,"Could not open common message queue %d",key); 
		goto m_return;
	}
	msq_common = r;
	...
	if (pid > 0) exit(0);
	sid = setsid();
	if (sid < 0) { /*printf("sid");*/ TRACE(TLVL_DEBUG,"sid"); exit(errno); }
	umask(0);
	...
	//gaibu_msg(MNOTICE,gaibu_msg_buf);
	TRACE(TLVL_INFO,"OV DAQ Process Started"); 
#endif
...
start_over:
	...
	if (r < 0) {
		...
		//gaibu_msg(MNOTICE, gaibu_msg_buf);
		TRACE(TLVL_DEBUG,"Failed to initialize libusb"); 
		goto m_return;
	}
	...
	if (r < 0) {
		...
		//gaibu_msg(MNOTICE, gaibu_msg_buf);
		TRACE(TLVL_DEBUG,"libusb: Could not get list of devices"); 
		goto m_deinit;
	}
	...
	while ((dev = devs[i++]) != NULL) {
		...
		if (r < 0) {
			...
			//gaibu_msg(MNOTICE,gaibu_msg_buf);
			TRACE(TLVL_DEBUG,"libusb: Could not open device"); 
                        usbsystemdevice--;
			continue;
		}
		...
		if(j == 0) {
			...
			//gaibu_msg(MNOTICE, gaibu_msg_buf);
			TRACE(TLVL_DEBUG,"Found USB readout board without any serial number"); 
			goto m_find_release;
		}
		...
		if (r < 0) {
			...
			//gaibu_msg(MNOTICE, gaibu_msg_buf);
			TRACE(TLVL_DEBUG,"libusb: Getting serial number problem number: %d",r); 
			goto m_find_release;
		}
		...
		TRACE(TLVL_DEBUG,"Found USB readout board serial #%d", ser_no);
		usbsystemdevice++;
		...
		if (r < 0) {
		  ...
		  TRACE(TLVL_DEBUG,"Com out: could not open message queue %d", key); 
		  goto m_find_release;
		}
		...
		if (pid < 0) {			// failed to fork
		  ...
		  TRACE(TLVL_DEBUG,"Failed to fork usb %d", ser_no);
		  goto m_find_release;
		  } else if (pid == 0) {	// child
		  goto mh_init;
		}
	...
	while (!kill_me) {
	  ...
	  if (timer_zero_reset(T_RESPAWN, 5)) {
	    if(usbsystemdevice%usb_counter!=0) {  
	      ...
	      //sprintf(gaibu_msg_buf,"Number of USBs connected = %d(%d)",usb_counter,usbsystemdevice);
	      //gaibu_msg(MERROR,gaibu_msg_buf);
	      TRACE(TLVL_DEBUG,"Error: number of USBs connected = %d(%d), starting over",usb_counter,usbsystemdevice); 
	      goto start_over;
	    }
	    //goto start_over;
	  }
...
mh_DEBUG:
	...
	//gaibu_msg(MNOTICE,gaibu_msg_buf);
	TRACE(TLVL_INFO,"OV DAQ USB Stream %d Started", ser_no); 
#endif
	...
	if (!com_out_transfer) { 
		...
		TRACE(TLVL_DEBUG,"libusb com out: not enough memory"); 
		goto mh_free0; 
	}
	...
	if (!per_out_transfer) { 
		...
		TRACE(TLVL_DEBUG,"libusb per out: not enough memory");
		goto mh_free1;
	}
	...
	if (!diag_in_transfer) {	
		...
		TRACE(TLVL_DEBUG,"libusb diag in: not enough memory");
		goto mh_free2; 
	}
	...
	for (i = 0; i < BUF_NO; i++) {
		...
		if (d[i].buf_state == BUF_WRITING) {
			...
			TRACE(TLVL_DEBUG,"aio cancel %d: %d", i, r); 
			d[i].buf_state = BUF_NOTHING;
		}
	}
...
mh_free0:
	...
	if (r < 0) { 
		...
		TRACE(TLVL_DEBUG,"libusb couldn't clear stall ep_data_in %d", r); 
	}

	r = libusb_clear_halt(devh, EP_DIAG_IN);
	if (r < 0) { 
		//printf("couldn't clear stall ep_com_in %d\n", r); 
		TRACE(TLVL_DEBUG,"libusb couldn't clear stall ep_com_in %d", r); 
	}

	r = libusb_clear_halt(devh, EP_COM_OUT);
	if (r < 0) { 
		//printf("couldn't clear stall ep_com_out %d\n", r); 
		TRACE(TLVL_DEBUG,"libusb couldn't clear stall ep_com_out");
	}

	r = libusb_reset_device(devh);
	if (r < 0) { 
		//printf("couldn't reset device %d\n", r); 
		TRACE(TLVL_DEBUG,"libusb couldn't reset device %d", r);
	}
