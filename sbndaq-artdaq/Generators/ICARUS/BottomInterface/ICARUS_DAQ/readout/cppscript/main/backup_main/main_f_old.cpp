            ...
            try{
               ...
               // cout << "msgp = " << *((int *)msgp.mtext) << "\n";
                for(int i = 0; i<=MQ_size;i++){
                    msgsnd(MQs[0][i],(void*)&msgp,MSG_BUF_SIZE-1,IPC_NOWAIT | MSG_NOERROR);
    ...
    while(1){
        try{
            ...
            try{
                ...
                strcpy((char *)msgp.mtext, data);
                for(int i =0;i<=MQ_size;i++){
                    msgsnd(MQs[0][i],(void*)&msgp,MSG_BUF_SIZE,IPC_NOWAIT);    
...
void mq_send_usb (long msg, int usb_num, char data[]){
    int mq0;
    int mq1;
    struct mymsg msgp;
    memset(msgp.mtext,0,MSG_BUF_SIZE);
            ...
            try{
               ...
               // printf ("tmp = %d\n", tmp1);
                ...
                //    cout << "mq1 = " << MQs[1][i] << ",mq0 = " << MQs[0][i] << ",msg = " << msgp.mtype << ",data = " << msgp.mtext << "\n";
                    ...
                    //   int rc = msgsnd(MQs[0][i],(void*)&msgp,msgsz, IPC_NOWAIT | MSG_NOERROR); 
                    //   if(rc == -1){
                    //       perror("msgsnd");
                    //       exit(1);
                    //   }
...
void send_out_usb(int usb, long usbdata){
   // string data = to_string((unsigned long)usbdata);
    char data1[100];
    memset(data1,0,100);
   // strcpy(data1,data.c_str());
   // data1[0] = usbdata;
    memcpy(data1,&usbdata,sizeof(long));
...
void com_usb(int usb, int pmt, int b2, int b3){
    int b1 = 4194304+((pmt & 63) * 256 + b2) *256 + b3;
   // cout << "b2 = " << b2 << ", b3 = " << b3 << ", b1 = " << b1 << "\n";
    send_out_usb(usb, b1);
}

void set_inhibit_usb(int usb, long usbdata){
  //  string data = to_string((unsigned long)usbdata);
    char data1[32];
    memset(data1,0,32);
  //  strcpy(data1,data.c_str());
...
void usb_usb(int usb, int adr, long data){
    int b1 = (adr << 16) + (data & 65535);
    send_out_usb(usb,b1);
}

void usb(int adr, long data){
    int b1 = (adr << 16) + (data & 65535);
    send_out(b1);
}

void dac_usb(int usb, int pmt, long data){
...
void initializeboard(int define_runnumber, int trigger_num, int pmtini, int pmtfin){
    int disk_num = 1;
    string DataPath = "/OVDAQ/DATA/";
    string DataFolder;
    DataFolder = "../data" + to_string(disk_num) + DataPath;
    int pipedelay = 20;
    ...
    printf("DataPath=%s, Disk=%d\n",DataPath.c_str(),disk_num);
    
    //    set_data_path(DataPath.c_str());
    //check_diskspace;
    ...
    //    set_run_number(run_number.c_str());
    ...
    for (pmt1 = pmtini; pmt1 <= pmtfin; pmt1++){
        ...
        com_usb(usb_local, pmt_local, 87, 0b11);           // force readout -> 01: 1msec, 10: 16msec, 11: 256msec
    ...
    if(mode.compare("mysql") == 0){
                    ...
                    if(result)
                    {
                        ...
                        while(sql_row = mysql_fetch_row(result))
                        {
                            ...
                            if(pmtdata[usbtemp][pmttemp][2] == "-999"){
                                ...
                                structure[usbtemp] = 0;
                                usbhowmanyboards[1][usbhowmanboardscount] = usbtemp;
                                usbhowmanyboards[0][usbhowmanboardscount]++;
                                usbhowmanboardscount++;
                            }
                        }
                    }
                }
        ...
        if(!usb_board || !pmt_board)
        {
            ...
            if(!triggerbox){
                ...
                usbhowmanyboards[1][usbhowmanboardscount] = usb_board;
                usbhowmanyboards[0][usbhowmanboardscount]++;
                usbhowmanboardscount++;
            }
            else{
...
void test_system_time(int usb, int pmt){
    ...
    char run2[128];
    strcpy(run2,run1.c_str());
    set_run_number(run2);                     // define the run_number and pass it to the DAQ
...
void generatercsv2(int usb, int pmt)
{
    getpmtdata(usb, pmt);
    
    string full_path = "/e/h.0/localdev/readout/data1/OVDAQ/DATA/Run_" + run_number;
...
nohomedir:
    ...
    //parse data file
    //  string cmd = unpacksignal(file, pmt_board);
    string cmd = "test";
    ifstream infile1(cmd.c_str());
    ...
    else
    {
        ...
        fclose(PLOT);
        string plotstr = "/usr/bin/gnuplot plot.gp";
