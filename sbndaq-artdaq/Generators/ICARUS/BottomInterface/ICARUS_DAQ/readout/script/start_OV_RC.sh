...
export DCONLINE_MOUNT=/home/dchooz/dconline/trunk
...
if [ "$1" = "" ]; then
echo "Usage : ./start_OV_RC.sh <process name> <argument>"
else

if [ "$2" = "" ]; then
prog=$1
arg=""
pid=`ps aux | grep "$1" | grep -v "start_OV_RC.sh" | grep -v "emacs" | grep -v "grep" | grep -v "tail -f" | awk '{print $2}' `
else
prog=$1
arg=$2
pid=`ps aux | grep "$1 $2" | grep -v "start_OV_RC.sh" | grep -v "emacs" | grep -v "grep" | grep -v "tail -f" | awk '{print $2}' `
fi
...
echo "Starting process $1 at $tim" >> $DCONLINE_PATH/DCOV/log/"$1".log
`nohup perl $1 >> $DCONLINE_PATH/DCOV/log/"$1".log 2>>$DCONLINE_PATH/DCOV/log/"$1".log < /dev/null &`
