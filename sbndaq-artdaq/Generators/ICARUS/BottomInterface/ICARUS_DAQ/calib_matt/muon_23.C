...
void muon_23(const int subrunnumber=171)
{
  ...
  bool faster = true; //temporary, for faster running while testing with icarus
...
/*for(int i=0;i<channels_in.size();i++){
  ...
  TH2D *channel_events_bi = new TH2D("channel_events_bi", "Events per channel (bilayer hits)",64,0,64,numberofboardused,0,numberofboardused);
  TH2D *channel_events_mono = new TH2D("channel_events_mono", "Events per channel (single strip)",64,0,64,numberofboardused,0,numberofboardused);
  ...
  for (int j=1; j<=numberofboardused; j++) //to be replaced after query on mysql
    {
	  ...
	  //cout << "pmtbdnum[j]: " << pmtboardnumber[j] << ", i: " << i << ", cutnpe: " << cutnpe[pmtboardnumber[j]][i] << endl; 
          //cutnpe[pmtboardnumber[j]][i] = 0;
	}
    }
...
*/
  ...
  for(int p=0; p<numberofboardused; p++){ //loop over pmt boards
	...
	channel_events_bi->Fill(strip_bot_m,p);	
	channel_events_bi->Fill(strip_top_m,p);	
	ADC[pmtboardnumber[p+1]][(int)strip_top_m]->Fill(adc_top_m);
	ADC[pmtboardnumber[p+1]][(int)strip_bot_m]->Fill(adc_bot_m);
	//TESTING  
    ...
    }//end of module if statement for each module
  }
}

for(int i=0; i<numberofboardused; i++){
  cout << "Events in board " << pmtboardnumber[i+1] << ": " << blcount[i] << endl;
}
  cout << "Hits in first module pair, x-direction: " << module_coinc->GetEntries() << endl;       // 19.000
  cout << "Hits in first module pair, y-direction: " << module_coinc2->GetEntries() << endl;       // 19.000
  cout << "Hits in second module pair, x-direction: " << module_coinc3->GetEntries() << endl;       // 19.000
  cout << "Hits in second module pair, y-direction: " << module_coinc4->GetEntries() << endl;       // 19.000
