...
loadrunsummarypostgres checkHVstatus checkLVstatus test_system_time check_system_time where_to_run is_readout_alive
updaterunsummarypostgres rc_runend_comment loadconfig OVCalibSummary updateOVCalibSummary checkOVCalibend
...
$meanph $meanpe $meangain $meanentries $laserrate @dacvalue $takebaseline $postgresload
...
$OVSERV_HOST $OVSERV_IP $postgres_table
...
$onlinetable $onlinetableGains $run_comment $run_number $run_type $run_length $run_OVtype $run_shifter
...
our ($client_sock,$client_sock_OVRC,$DebugDAC,$pmt,$usb,$pmtserialnumber,$HVsetting,$lasersinglepe,$postgresload,
$DACt,$thresh,$hdelay,$runlength,$newrunlength,$pipedelay,$force_trig,$meanpulseheight,$gate,$gateonoff,$elapsed_time,$postgres_table,
...
our $onlinetable = "online400"; # this is the default table for the timing test# "online800";
our $onlinetableGains = "online400Gains";
...
my $postgresSocket = "/var/lib/postgres/postgres.sock"; #TODO: Detect whether to use this socket based on whether artdaq is set up?
...
$DBhostname = "ifdb08.fnal.gov";
$user = "icarus_reader";
$password = "icarus4read";
$database = "icarus_hardware_prd";
...
#	    print "Found run type !\n";
	...
	if($found == 1){
		...
		#here I will decide what postgres table for configuration I would use
		#$run_type = $1;
		$onlinetable = $1;
	    }
...
my $mode_local = shift;
if(!$mode_local) { $mode = "postgres"; }
else { $mode = $mode_local; }
@usblocal = ();

$totalbox = 0;

#code for retrieving info from postgres database
...
eval {
	$dsn = "dbi:Pg:database=$database;host=$DBhostname;port=5453"; 
	$dbh = DBI->connect($dsn, $user, $password); 
	print "Failed to connect to database: $@\n" if $@;
	$drh = DBI->install_driver("Pg");
	$sth = $dbh->prepare("SELECT USB_serial, $onlinetable.pmt_serial, board_number, HV, DAC_threshold,                                      
	use_maroc2gain, gate, $onlinetable.pipe_delay, trigger_mode, force_trig, pmt_board_id, $onlinetableGains.strip, $onlinetableGains.gain FROM 
	$onlinetable, $onlinetableGains WHERE $onlinetable.pmt_serial=$onlinetableGains.pmt_serial");      ##CM this was USB_Serial ...10022018
	$sth->execute();
	...
	while (my $ref = $sth->fetchrow_arrayref()) {
		...
		$pmtdata[$ref->[0]][$ref->[2]][9] = "postgres mode";  #comments
		...
		if($pmtdata[$ref->[0]][$ref->[2]][2] eq "-999") {
		    ...
		    for(my $index=1;$index<65;$index++){
		        if($ref->[11] eq $index) {
			    print "$index\n";
		            $gaindata[$ref->[0]][$ref->[2]][$index] =  $ref->[12];
			    if($index eq 1) {
				print "GOT HERE\n";
   ...
   print "Reading from postgres failed: $@\n" if $@;
   if(!$@) { 
#	print "\nLoading settings from postgres table: $onlinetable\n\n";
        $postgresload = 1.; 
   }
...
if ($gate eq "on"){
    ...
    $gateonoff = 0b01010;
    #$trigger_mode = 0b00010000; #Don't force trigger_mode to be a hardcoded value anymore and update postgres tables accordingly.  
}
...
for(my $index=0;$index<64;$index++){
	...
	#print "\nusbboard: $usbboard\n";
	#print "pmtnumber: $pmtnumber\n";
	#print "index: $index\n";
	#print "gaindata[usbboard]: $gaindata[$usbboard]\n";
	#print "gaindata[usbboard][pmtnumber]: $gaindata[$usbboard][$pmtnumber]\n";
	#print "gaindata[usbboard][pmtnumber][index]: $gaindata[$usbboard][$pmtnumber][$index]\n";
    }
...
###now let's initialize everything for the data taking
	...
	elsif($usemaroc2gainconstantsmb eq "yes" && $postgresload == 0){ 
        	print "Error cannot load postgres gain constants from postgres\n";
        	com_usb $usb_local, $pmt_local, 74, 0b0100000;               # default gain 
        }
	elsif($usemaroc2gainconstantsmb eq "yes" && $postgresload == 1){
   		for(my $index=1;$index<65;$index++){
			com_usb $usb_local, $pmt_local, $index - 1, 2*$gain[$index]; # applying gain constants from postgres
    		}
	}
...
#loadconfig("postgres");         #this will load the configuration from postgres;

loadconfig("postgres",$usb,$pmt);
...
eval {
  $dsn = "dbi:Pg:database=$database;host=$DBhostname;postgres_socket=$postgresSocket";

  $dbh = DBI->connect($dsn, $user, $password);

  $drh = DBI->install_driver("postgres");  
  ...
  if($@){
   print "Failed to import into postgres info for this run: $@\n" if $@;
   gaibu_msg($MWARNING, "OV DAQ: Failed to import intial run info inside postgres");
  }

}

##################################################################################################
sub updaterunsummarypostgres {
...
#error message
 if($@){
   print "Failed to import into postgres info for this run: $@\n";
   gaibu_msg($MWARNING, "OV DAQ: Failed to update run info inside postgres");
 }
...
#error message
 if($@){
   print "Failed to import into postgres Calib info: $@\n";
   gaibu_msg($MWARNING, "OV DAQ: Failed to update Calib info inside postgres");
 }
...
#error message
 if($@){
   print "Failed to update into postgres the Calib info: $@\n";
   gaibu_msg($MWARNING, "OV DAQ: Failed to update Calib info inside postgres");
 }
...
sub checknumusbs{
	...
	print "Expected from postgres: $expected\tFound with lsusb: $usbcnt\n";
	#gaibu_msg($MERROR,"Number of USB found in the system inconsistent with postgres config");
	#exit(0);
    }
...
my $dsn = "dbi:Pg:database=$HVdatabase;host=$DBhostname;postgres_socket=$postgresSocket";
...
#	print "number of row read $#row \n";
	...
	if($#row < 16){
	    ...
	    updaterunsummarypostgres("HV","und");
...
if( ($nChanOn) == ($nChanGood) ){
	updaterunsummarypostgres("HV","ON");
#	print "OVHV is ON and Working !\n";
}
if( ($nChanBad) > 0){
        updaterunsummarypostgres("HV","BAD");
...
if ( $nChanOff == $nChan )
{
    ...
    updaterunsummarypostgres("HV","OFF");
...
sub check_rate {
   ...
   print "Getting baselines from files @files.\n";
	...
	foreach $tempfile (@files) {
	    ...
	    open IN, "/usr/bin/perl -S /home/bcrt/readout_linux/macro/decode.pl \"${tempfile}\" |";
	    while(<IN>) {
		chomp ;
		next if /^\s*$/;
		my @line = split /\s*,\s*/;
...
#		sendmail("OVRC","CRITICAL","PMT Board $pmttoboard[$i] is missing from the baseline file");
	...
	print "Error: Number of pmts found: $tmptotpmts\tNumber of pmts expected in postgres: $tot_pmt\n";
#	gaibu_msg($MERROR,"Error: Number of pmts found: $tmptotpmts, Number of pmts expected in postgres: $tot_pmt");
...
#    loadconfig("postgres");   # at this point $totusbs->contains number of usb from postgres
    # $usb_box is the trigger box USB

#print "Number of USB found in postgres connected to PMT board= $totusbs, the trigger box is on USB $usb_box and we have $totalbox trigger box\n";
...
my $DBhostname="dcfpostgres.in2p3.fr";

my $dsn = "dbi:Pg:database=$LVdatabase;host=$DBhostname;postgres_socket=$postgresSocket";
...
#    $myquery = sprintf("select * from %s where time>=\'%s\' and time<=\'%s\'",$tableName, $earlyTime, $currTime);
	...
	#updaterunsummarypostgres("LV","monitor off");                                                           
	$restart_required = 1;
	goto end;
    }
...
if( ($nChanOn) > 5 ){                                                                                        
#    updaterunsummarypostgres("LV","ON");                                                                        
    #print "\nOVLV is ON and Working !\n";                                                                      
    if( ($nChanGood) < 5){                                                                                   
#        updaterunsummarypostgres("LV","ONw");                                                                   
...
#    updaterunsummarypostgres("LV","OFF");                                                                       
}                                                                                                            
...
#code for retrieving info form postgres database
	...
	eval {
	    ...
	    print "Failed to connect to database: $@\n" if $@;
	    if($@ eq "") {print "Connected to postgres database\n\n"; }
	...
	print "Reading from postgres failed: $@\n" if $@;
#if($@ eq "") {print "Setting read out from postgres server @ Nevis\n\n";}

  
	if($foundcount == 0){
	    print " \t\t !!!!!! Warning !!!!!!!\n";
	    print "Info for your PMT <-> board not present in the postgres database or do not match.\n\n";
	    print "For PMT = $pmtserialnumber the board address in the postgres database is not $pmt\n";
	    $postgresload = 0;
	  restartchoice:
		...
		setpmtdata($usb,$pmt);
		if($pmtdata[$usb][$pmt][4] eq "yes") {
		    print "Please type which gain constants you would like to use (postgres or local)\n";
		    chomp($choicesecond = <STDIN>);
		    if($choicesecond eq "local"){ $postgresload = 0;};
		}
	    }
	}
...
#print "Baseline decoded for pmt $pmt...\n\n";
	...
	$gateonoff = 0b01010;
	#$trigger_mode = 0b00010000; #trigger_mode now comes from the postgres table instead
    }
    elsif( $gate eq "trigger" ){ 
	$gateonoff = 0b01011;
...
#tarry 2.5;                          # seconds
    ...
    elsif($usemaroc2gainconstantsmb eq "yes" && $postgresload == 0 && $equalization eq "false"){ 
...
#print "Local file with the Maroc2 gain constants found, now reading and applying settings...\n\n";
    ...
    elsif($usemaroc2gainconstantsmb eq "yes" && $postgresload == 1 && $equalization eq "false"){
	open OUT, ">$full_path2/PMT_${pmt}/gain.txt" or die $!;
	for(my $index=1;$index<65;$index++){
	    com_usb $usb, $pmt, $index - 1, 2*$gain[$index];
	    #print "postgres gain constants found: $index, $gain[$index+1] \n";
	    print OUT "$index \t $gain[$index] \n";       
	}
	print "postgres gain constants applied. \n";
	close OUT or die $!;
    }
...
#print "Creating summary file for PMT $pmt...append info if file already exists\n";
  ...
  if($equalization ne "true"){ loadrunsummarypostgres(); };         #copy info into the postgres database
}
################################################################################################################################
