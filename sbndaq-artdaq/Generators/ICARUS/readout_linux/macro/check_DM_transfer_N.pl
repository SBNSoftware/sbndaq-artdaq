...
#NOW the DF check date and size
		...
		while (my $ref3 = $sth3->fetchrow_arrayref()) {
		    ...
		    $DF_file = "/dcnovmon${DF_PATH}${DF_NAME}";
			...
			print "Run = $Run_number -- DF File /dcnovmon${DF_PATH}${DF_NAME} not found \n";
			$needtorunDF = 1;
			$needtorun = 1;
		    }
...
#check if the DF file exist, maybe DF failed and that's it
    if(-e "/dcnovmon/data1/OVDAQ/DATA/DCRun${Run_number}_RAW_v17_OV_muon.root") {
	print "DF File Found\n";
	$DFok = 1;
	goto fileDFexist;
    }
    
    if(-e "/dcnovmon/${EB_path}DCRunN${Run_number}OVDAQ.tar.gz" or -e "/dcnovmon/${EB_path}Run_${Run_number}") {

	if(-e "/dcnovmon/${EB_path}DCRunN${Run_number}OVDAQ.tar.gz") {
	    print "Unzipping file /dcnovmon/${EB_path}DCRunF${Run_number}OVDAQ.tar.gz \n";
	    system qq| cd /dcnovmon/${EB_path}; tar -xvf DCRunN${Run_number}OVDAQ.tar.gz|;
	}
	
	if(-e "/dcnovmon/${EB_path}Run_${Run_number}") {
	    $needtozip = 1;
	}
#rerun the DF
	eval {
...
fileDFexist:
	...
	#   at this point if no more file and the run is ended already make a system call and zip the EB_path/Run_number     
		system qq| cd /dcnovmon/data1/OVDAQ/DATA/; time tar -cvzf DCRunN${Run_number}OVDAQ.tar.gz Run_${Run_number}/processed/*.bin|;
		system qq| cd /dcnovmon/data1/OVDAQ/DATA; time rm -r Run_${Run_number}|;
...
#DF was not successfull # do not re-delete the data send an email
	...
	if($needtozip == 0) {
#now try to run the EB on dcnovmon
	    eval {
		...
		if(!defined($stop_time)) { 
		    $dbh4->do("UPDATE doublechooz_ov_nd.OV_runsummary set stop_time = '25:25:25' where Run_number = ${Run_number}"); 
		}
		$dbh4->do("DELETE from doublechooz_ov_nd.OV_ebuilder where Run_number = ${Run_number} and Entry =0");
		$dbh4->disconnect();
	    };
	    
#need to check that all binary files has been moved to the decoded folder
	    if($DAQdisk == 1) {
		my $binpath = "/dcnovdaq/data1/OVDAQ/DATA/Run_${Run_number}";
...
#first remove .wr for file that have that exstension
		...
		my $binpath = "/dcnovdaq/data2/OVDAQ/DATA/Run_${Run_number}";
		print "Run=$Run_number, Binary Path:${binpath}/binary \n";
		open IN, "cd ${binpath}/binary ; ls -ltr --time-style=long-iso *|";
		while(<IN>) {
		    if(/ \S+ \d+-\d+-\d+ \d+:\d+ (\d+)_(\d+)/) {
	    ...
	    print "Now try to run EB on dcnovmon with the following parameters: $Run_number, $EBthreshold, $EBtriggertype, $EBruntype \n";
	    sleep 10;
	    system qq|time ssh dconline\@dcnovmon.in2p3.fr "${online_path}\/DCOV\/readout\/startupscript\/start_OV_tools_nohup.sh ${remote_path} EventBuilder 0 1 ${Run_number} ${EBthreshold} ${EBtriggertype} ${EBruntype} dcnovserv.in2p3.fr 1"|;	    
	    #move the data from /dcnovmon/OVDAQ/REP to /dcnovmon/OVDAQ/DATA
	    system qq| mv /dcnovmon/data1/OVDAQ/REP/Run_${Run_number}/T2ADC0073P100P200/Run_${Run_number} /dcnovmon/data1/OVDAQ/DATA/. |;
	    system qq| rm -rf /dcnovmon/data1/OVDAQ/REP/Run_${Run_number} |;
	    #run the DF
	    $needtozip = 1;
	    goto tryagainDF;
	}
...
#check if DOGSfier finish ok                                                                                                   
    ...
    if(-e "/dcnovmon${EB_path}Run_${DF_Run_number}") {
        open IN, "ls /dcnovmon${EB_path}Run_${DF_Run_number}/ |" or die $!;
        while(<IN>) {
            next if $_ =~ /processed/;
            ++$DF_file_count;
        }
