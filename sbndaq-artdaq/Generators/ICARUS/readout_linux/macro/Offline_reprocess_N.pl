...
eval {
	...
	if(defined($ref->[0])) { #check if the flag is defined
		...
		if($ref->[2] == -1 || $ref->[2] == 0 || !defined($stop_time)){   #EB was not running or EB exited with Fatal
		    ...
		    my $binpath = "/dcnovdaq/data$DAQdisk/$DAQFolder/Run_$Run_number/binary";
...
#		    }
			...
			my $decodedpath = "/dcnovdaq/data$DAQdisk/$DAQFolder/Run_$Run_number/decoded";
...
#				print "Debug: decoded usb = $2, timestamp = $1\n";
			...
			unless(-e "/dcnovmon/data1/OVDAQ/REP/Run_${Run_number}") {
			    print "Folder not found - creating folder \n";
			    system qq| mkdir /dcnovmon/data1/OVDAQ/REP/Run_${Run_number}; chmod 777 /dcnovmon/data1/OVDAQ/REP/Run_${Run_number} |;
			}
		    }
...
#check if DOGSfier finish ok
	...
	if(-e "$logfile") {
		...
		system qq| mv /dcnovmon${EB_path}Run_${DF_Run_number}/DCRunF${DF_Run_number}* /dcnovmon${EB_path}Run_${DF_Run_number}/processed/.|;  #move last file to processed folder
	    }
	}    
	else {
