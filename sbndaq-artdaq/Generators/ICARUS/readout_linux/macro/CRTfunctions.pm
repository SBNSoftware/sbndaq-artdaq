...
sub generatebaseline {
  ...
  #  printf("Baseline file: $filebase\n");
    ...
    system qq|perl -S /home/bcrt/readout_linux/macro/baselines.pl "${DataFolder}/Run_${run}/binary/" "baseline_${usb}" "${pmt}"|;
...
#my @files = <${DataFolder}/Run_${run}/decoded/*_${usb}.done>; #exclude baselines
	...
	system qq|/usr/bin/perl -S /home/bcrt/readout_linux/macro/decode.pl "$tempfile" >> "${filename}"|;
    }
}
...
# parse data file
open IN, "/usr/bin/perl -S /home/bcrt/readout_linux/macro/unpacksignal.pl \"$file\" \"$pmt_board\" |" or die $!;
...
system("/bin/gnuplot plot.gp") == 0 or die "Error running gnuplot: $?";
