...
{
  printf("Usage: ");
  printf("confsetbias input_SCRfile.txt bias output_SCRfile.txt\n");
...
void ConfigSetBit(uint8_t *buffer, uint16_t bitlen, 
		  uint16_t bit_index, int value)
{
  uint8_t byte;
  uint8_t mask;
  byte=buffer[(bitlen-1-bit_index)/8];
...
void ConfigSetGain(int chan, uint8_t val)
{
  ...
  for(int b=0;b<6;b++)
  {
    if((val & mask)>0) ConfigSetBit(bufSCR,1144,619+chan*15+b,1); 
    else ConfigSetBit(bufSCR,1144,619+chan*15+b,0);
    mask=mask>>1;
  } 
}
...
void ConfigSetBias(int chan, uint8_t val)
{
  ...
  for(int b=0;b<8;b++)
  {
    if((val & mask)>0) ConfigSetBit(bufSCR,1144,331+chan*9+b,1); 
    else ConfigSetBit(bufSCR,1144,331+chan*9+b,0);
    mask=mask>>1;
  } 
}

// write CITIROC SC bitstream to file, buf[MAXPACKLEN]
int WriteBitStreamAnnotated(const char * fname, uint8_t *buf, int bitlen) 
{ 
  ...
  for(int Ch=0; Ch<32;Ch++) 
  {
    fprintf(file,"%c%c%c%c \' Ch%d 4-bit DAC_t ([0..3])\'\n",ascii[ib],
	    ascii[ib+1],ascii[ib+2],ascii[ib+3],Ch);
    ib+=4;
  }
  ...
  for(int Ch=0; Ch<32;Ch++) 
  {
    fprintf(file,"%c%c%c%c",ascii[ib],ascii[ib+1],ascii[ib+2],ascii[ib+3]); 
    ib+=4;
    fprintf(file,"%c%c%c%c",ascii[ib],ascii[ib+1],ascii[ib+2],ascii[ib+3]); 
    ib+=4;
  ...
  fprintf(file,"%c%c%c%c ",ascii[ib],ascii[ib+1],ascii[ib+2],ascii[ib+3]); 
  ib+=4;
  fprintf(file,"%c%c%c%c \' 10-bit DAC1 (MSB-LSB): 00 1100 0000 for 0.5 p.e.\'\n",ascii[ib],ascii[ib+1],ascii[ib+2],ascii[ib+3]); 
  ib+=4;

  fprintf(file,"%c%c ",ascii[ib],ascii[ib+1]); 
  ib+=2;
  fprintf(file,"%c%c%c%c ",ascii[ib],ascii[ib+1],ascii[ib+2],ascii[ib+3]); 
  ib+=4;
...
int main (int argc, char **argv)
{
  ...
  if(bitlen==1144) 
  {
    printf("ReadBitStream: %d bits read from SCR config file %s.\n",
	   bitlen,argv[1]);
  }
  else 
  { 
    printf("ReadBitStream: %d bits read from unrecognized type file %s. Aborting\n",bitlen,argv[1]);
    return -1; 
  }
  
  printf("Setting bias for all 32 channels to %d.\n",bias);
  for(int i=0;i<32;i++) 
  { 
    printf("ch %d was %d, set to %d\n", i,ConfigGetBias(i) , bias); 
    ConfigSetBias(i, bias);
  }
