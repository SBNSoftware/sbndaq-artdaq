...
bool sbndaq::CAENV1730Readout::checkHWStatus_(){
  ...
  for(size_t ch=0; ch<CAENConfiguration::MAX_CHANNELS; ++ch)
  {
	...
	if ( ch_temps[ch] > fCAEN.maxTemp ) 
	{
	  ...
	  // 2. Temperature > Requirement , 70C for V1730 and 85C for V1730S
	  // 3. Temperature < non_physical temperature, 200C since S/N 164 can produce non-physical temperature
	  if ( ch_temps[ch] < V1730_UNPHYSICAL_TEMPERATURE )
	  {
	  ...
	  {
	    // Ignore unphysical temperatures from S/N 164.  CAEN advises not to read temperatures 
	...
	TLOG(TLVL_WARNING) << "CAENV1730 BoardID " << fBoardID << " : "
			       << " Channel " << ch
			       << " temperature readout error " << ch_temps[ch] << " degrees Celsius."
			       << " ReadTemperature Return Code = " << retcod
			       << TLOG_ENDL;
      }
    }
