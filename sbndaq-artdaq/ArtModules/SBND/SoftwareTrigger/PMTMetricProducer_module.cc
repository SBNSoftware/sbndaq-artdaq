...
void sbnd::trigger::pmtSoftwareTriggerProducer::produce(art::Event& e)
{
  ...
  // TLOG(TLVL_ERROR) << "reference time stamp is " << refTimestamp << " ns";
  ...
  if (refTimestamp!=0) {    
    ...
    std::vector<uint32_t> wvfm_sum(fWvfmNominalLength, 0);
      ...
      // TLOG(TLVL_ERROR) << "windowStartBin" << windowStartBin;
      // TLOG(TLVL_ERROR) << "windowEndBin" << windowEndBin;
    ...
    if (fVerbose==1){
      ...
      if (fFindFlashInfo){
        ...
        // TLOG(TLVL_ERROR) << "flash prelim pe: " << flash_prelimPE ;
        // TLOG(TLVL_ERROR) << "flash prompt pe: " << flash_promptPE ;
        // TLOG(TLVL_ERROR) << "flash peak pe: "   << flash_peakPE ;
        // TLOG(TLVL_ERROR) << "flash peak time: " << flash_peaktime << " us" ;
      }
    }
  ...
  TLOG(TLVL_ERROR) << "end of pmtsoftware trigger";
