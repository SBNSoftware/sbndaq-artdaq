...
public:
  struct Config
  {
    ...
    fhicl::Sequence<int> fit_range_vec {
...
private:
  ...
  void PeakFindFit(TH1F* h, bool save, art::TFileDirectory tfdir, 
                   float &gain, float &gain_err, float &pedestal, float &pedestal_err, float &chisqr, int &ndf);
...
void sbndaq::CRTSinglePEAna::endJob()
{
  // store the histogram
  if(fverbose)
    std::cout << "Ending CRTSinglePEAna...\n";
  ...
  if(fFitADCRawHistos)
    {
      ...
      int FEB_id, channel;
      float gain, gain_err, pedestal, pedestal_err, chisqr;
      int ndf;
	  ...
	  for(auto iChannel=0; iChannel<(int)adc_1d_ditributions_perChannel_perFEB.at(iFeb).size(); iChannel++)
	    {
	      ...
	      if(fverbose)
		std::cout<<"FEB: "<<fFEBModules_vec.at(iFeb)<<", iChannel: "<<iChannel<<", gain "<<gain<<", pedestal "<<pedestal<<std::endl;
		...
		{
		  if(fverbose)
		    std::cout<<"WARNING: check out FEB: "<<FEB_id<<", channel: "<<channel<<std::endl;
  ...
  for(int thisFeb_no=0; thisFeb_no<fFEB_total_number; thisFeb_no++)
    {
      if(thisFeb_no<fFEB_total_number-1)
	std::cout<<fFEBModules_vec[thisFeb_no]<<",";
      else
	std::cout<<fFEBModules_vec[thisFeb_no];
    }
  std::cout<<" ]"<<std::endl;
...
void sbndaq::CRTSinglePEAna::MakeSinglePETree()
{
  ...
  if(nhits<=0)
    {
      std::cout << " no crt hits in event " << fEvent << std::endl; return;}
  ...
  if(hitcount>1)
    std::cout << "event " << fEvent << " has " << hitcount << " crt hits " << std::endl;
}// end of MakeSinglePETree.

void sbndaq::CRTSinglePEAna::PeakFindFit(TH1F* h, bool save, art::TFileDirectory tfdir, float &gain, float &gain_err, float &pedestal, float &pedestal_err, float &chisqr, int &ndf)
{  
  float gainSeed = fbias_voltage*fgain_increment_slope;

  Double_t tSpectrumSigma = 1.85;
  Double_t tSpectrumThreshold = 0.18;
  if(fbias_voltage>200.) tSpectrumThreshold = 0.14;
  ...
  fit_canvas->cd(1);
  h->GetXaxis()->SetTitle("ADC"); h->GetYaxis()->SetTitle("Hits"); h->SetStats(kFALSE);  
  h->Draw("e0hist");
  ...
  for(int ipeak=0; ipeak<nPeak; ipeak++)
    { // store the peak value from TSpectrum in the vector. 
      if(peaks[ipeak]<0)
	std::cout << "bad peak value: " << peaks[ipeak] << std::endl;
  ...
  for(int ipeak=0 ; ipeak<nPeak && ipeak<(int)nPeakMax; ipeak++)
    {
      ...
      if(ipeak==0)
	{ // remove pedestals. 
	  if(adc_value[ipeak+1]-adc_value[ipeak]<gainSeed*0.7 || (adc_value[ipeak+1]-adc_value[ipeak])>gainSeed*1.5)
	    continue;
	}
      ...
      if(adc_value[ipeak]>h->GetBinLowEdge(1)+15 && adc_value[ipeak]<h->GetBinLowEdge(h->GetNbinsX())-15 && abs(adc_value[ipeak]-gfit->GetParameter(1))<15)
	{
	  ...
	  if(ipeak!=0 && ipeak!=nPeak-1 //check it's not first or last peaks
	     && (adc_value[ipeak+1]-adc_value[ipeak]<gainSeed*0.7 || adc_value[ipeak]-adc_value[ipeak-1]<gainSeed*0.7))
	    { //check peak within 30% of expected gain w.r.t adj.
	      if(fverbose)
		std::cout << "determined peak " << ipeak << " (" << adc_value[ipeak] << " ADC) is false peak. removing..." << std::endl;
	    ...
	    { //if not missed peak, could there have been a skipped peak?
	      if(ipeak!=0 && ipeak!=nPeak-1 && adc_value[ipeak+1]-adc_value[ipeak]<gainSeed*1.2 && adc_value[ipeak]-adc_value[ipeak-1]>gainSeed*1.5)
		{ //missed peak adjust
		  if(ipeak==1 && (adc_value[ipeak]-adc_value[ipeak-1])>gainSeed*1.5)
		    {  // remove pedestals. 
		      for(int j=ipeak; j<nPeak; j++) peak_value[j] = peak_value[j]-1;
		      if(fverbose)
			std::cout << "determined peak " << ipeak-1 << " (" << adc_value[ipeak-1] << " ADC) is pedestal. removing..." << std::endl;
		    }
		  else
		    {
		      for(int j=ipeak; j<nPeak; j++) peak_value[j] = peak_value[j]+1;
		      if(fverbose)
			std::cout << "missed peak detected. shifting peaks " << ipeak << " and higher up by 1..." << std::endl;
		    }
		}
  ...
  if(isBrokenChannel)
  ...
  else
    {
      ...
      gr_mean->GetXaxis()->SetTitle("Peak No.");
	...
	std::cout << "gain fit..." << std::endl;
      ...
      TPaveText *pt = new TPaveText(.15, .7, .4, .8, "NDC");
      pt->AddText(Form("#chi^{2}/dof: %.4f / %d", chisqr, ndf));
      pt->AddText(Form("Eff Gain: %.2f #pm %.3f", gain, gain_err));
